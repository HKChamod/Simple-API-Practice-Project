...
let db = new sqlite3.Database(DBSOURCE, (err) => {
  if (err) {
    ...
    db.run(
      "CREATE TABLE customers( id INTEGER PRIMARY KEY AUTOINCREMENT, customerName text, customerAddress text, email text, birthDate text, gender text, age INTEGER, cardHolderName text, cardNumber INTEGER, cardExpirytDate text, cvv INTEGER, timestamp text)",
      (err) => {
        if (err) {
          // Table already created
        } else {
