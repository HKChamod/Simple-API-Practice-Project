...
app.post("/api/customers/", (req, res, next) => {
    try {
      ...
      if (!cardNumberRegex.test(cardNumber)) {
        errors.push("Invalid credit card number (should be 12 digits)");
      }
      ...
      var sql =
        "INSERT INTO customers (customerName, customerAddress, email, birthDate, gender, age, cardHolderName, cardNumber, cardExpirytDate, cvv, timestamp) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
      var params = [
        customerName,
        customerAddress,
        email,
      ...
      db.run(sql, params, function (err, result) {
        if (err) {
          ...
          const message = `customer ${customerName} has registered`;
          res.status(201).json({
            message: message,
            id: this.lastID,
            data: req.body,
  ...
  app.put("/api/customers/", (req, res, next) => {
    ...
    db.run(
      `UPDATE customers set customerName = ?, customerAddress = ?, email = ?, birthDate = ?,gender=?,age=?,cardHolderName=?,cardNumber=?,cardExpirytDate=?,cvv=?,timestamp = ? WHERE id = ?`,
      [
        id,
        customerName,
        customerAddress,
      ...
      function (err, result) {
        ...
        res.status(201).json({ updated: this.changes });
      }
    );
  });
